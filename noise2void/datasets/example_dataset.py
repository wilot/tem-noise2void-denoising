...
class ExampleVideoDataset(Dataset, MultiChannelDataset):
    ...
    def _find_data_filepaths(cls) -> list[MultiChannelMetadata]:
        ...
        sample_filegroups: dict[int, dict[Channel, Path]] = dict()  # Uniquely identified by an ID number

        for fpath in cls.DATA_DIRECTORY.glob("*.dm4"):

            index, channel_str = fpath.stem.split("_")
            if index not in sample_filegroups:
                sample_filegroups[index] = {Channel(channel_str): fpath}
            else:
                if channel_str not in sample_filegroups[index]:
                    sample_filegroups[index][channel_str] = fpath
                else:
        ...
        for index in sample_filegroups:
            meta = MultiChannelMetadata(  # Will calculate the frame numbers later
                sample_filegroups[index], [Channel.HAADF, Channel.BF], "Example", index,
                None
            )
